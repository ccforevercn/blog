<template>
  <div class="app-container">
    ...
    <el-menu
      :default-active="where.category_id"
      mode="horizontal"
      @select="whereCategoryId"
    >
    ...
    >
        ...
        <template slot-scope="scope"
          ><span class="cursor"
            ><el-tag
              type="success"
              effect="plain"
              @click="cp(labelPrefix + scope.row.select)"
              >{{
                labelPrefix + scope.row.select
              }}&nbsp;&nbsp;&nbsp;复制</el-tag
            ></span
          ></template
        >
      ...
      <el-table-column label="配置值">
        <template slot-scope="scope">
          <span class="cursor-pointer" v-if="scope.row.type === 4"
            ><el-image
              class="table-image"
              @click="openImage(imageUrl + scope.row.value)"
          ...
          <span v-else>{{
            scope.row.value
              | formatValueFilter(scope.row.type_value, scope.row.type, that)
          }}</span>
        </template>
      </el-table-column>
      <el-table-column label="配置类型">
        <template slot-scope="scope"
          ><el-tag class="cursor" :type="scope.row.type | typeColorFilter">{{
    ...
    >
      <el-form class="edit-dialog-form">
        ...
        <el-form-item label="唯 一 值">
          <el-input
            v-model="editInfo.select"
            :readonly="editInfo.selectReadonly"
          />
        </el-form-item>
        <el-form-item label="配置分类">
          <el-select
            v-model="editInfo.category_id"
            placeholder="配置分类"
            :disabled="editInfo.categoryIdDisabled"
          >
            <el-option
        ...
        <el-form-item label="是否展示">
          <el-radio-group v-model="editInfo.is_show">
        ...
        <el-form-item label="配置类型">
          <el-radio-group
            ...
            :disabled="editInfo.typeDisabled"
            @change="setEditInfoTypeValueStatus"
          >
        ...
        <el-form-item
          v-show="editInfoValueRadioStatus || editInfoValueCheckBoxStatus"
          label="类 型 值"
        >
          <el-input
            v-model="editInfo.type_value"
            :readonly="editInfo.typeValueReadonly"
            placeholder="配置类型值格式：field:value|field:value..."
            @change="setEditInfoRadisOrCheckBoxValue"
          />
        </el-form-item>
        <el-form-item label="配 置 值">
          <el-radio-group
            v-show="editInfoValueRadioStatus"
            v-model="editInfo.value"
          >
            <el-radio
              v-for="item in radioOrCheckBoxValue"
              :key="item[0]"
              :label="item[0]"
              >{{ item[1] }}</el-radio
            >
          ...
          <el-checkbox-group
            v-show="editInfoValueCheckBoxStatus"
          ...
          <el-input
            v-show="editInfoValueInputStatus"
            v-model="editInfo.value"
          />
          <el-input
            v-show="editInfoValueTextareaStatus"
            v-model="editInfo.value"
            type="textarea"
          />
          <upload-image
            v-show="editInfoValueImageStatus"
            :images="image"
            :name="imageName"
            :path="imagePath"
...
import {
  ...
  PutUpdate,
  DeleteDelete,
} from "@/api/configmessage";
import { GetCategory } from "@/api/configcategory";
...
export default {
      ...
      if (type === 2) {
        ...
        if (value.length > 0) {
          ...
          for (const index in valueArr) {
            newValue +=
              that.formatConfigRadioOrCheckBoxvalue(values, valueArr[index]) +
              ",";
          }
  ...
  methods: {
    ...
    setEditInfoRadisOrCheckBoxValue() {
      ...
      this.radioOrCheckBoxValue = this.formatConfigRadioOrCheckBoxvalue(
    ...
    setEditInfoTypeValueStatus() {
      ...
      that.checkBoxValue = that.radioOrCheckBoxValue = [];
    },
    ...
    deleteDialog(index) {
      ...
      that
        .$confirm("您要永久删除【" + message.name + "】配置吗?", "提示", {
    ...
    editDialog(index) {
      ...
      this.editInfo.is_show = message.is_show.toString();
      ...
      this.dialogTitle =
        "修改【" + message.name + "】配置(可修改配置名称，配置值，配置描述)";

        ...
        <el-col :span="8"
          ><el-form-item label="账号"
            ><el-input
              ...
              class="where-username"
              @change="setWhereUserName" /></el-form-item
        ></el-col>
      </el-form>
    ...
    >
      ...
      <el-table-column label="管理员名称">
        <template slot-scope="scope"
          ><span>{{ scope.row.real_name }}</span></template
        >
      </el-table-column>
      ...
      <el-table-column label="管理员规则" align="center">
        <template slot-scope="scope"
          ><span
            >{{ scope.row.rulename
            }}{{ scope.row.rule_id > 0 ? "/" + scope.row.rule_id : "" }}</span
          ></template
        >
      </el-table-column>
      ...
      >
        <template slot-scope="scope"
          ><el-tag class="cursor" :type="scope.row.status | statusTypeFilter">{{
...
export default {
  ...
  methods: {
    ...
    deleteDialog(index) {
      ...
      that
        .$confirm("您要永久删除【" + admin.real_name + "】管理员吗?", "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning",
        })
        .then(() => {
          DeleteDelete({ id: admin.id }).then(() => {
            that.getList();
          });
        })
        .catch(() => {

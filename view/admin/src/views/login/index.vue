...
export default {
  ...
  methods: {
    getCaptcha() {
      var that = this;
      captcha().then((data) => {
        that.loginForm.key = data.key;
        that.captchaBase64 = data.url;
      });
    },
    showPwd() {
      ...
      that.$refs.loginForm.validate((valid) => {
        if (valid) {
          ...
          that.$store
            ...
            .catch((message) => {
              that.$message({ type: "error", message: message || "登陆失败" });
              that.loading = false;
            });
        } else {
...
.login-container {
  ...
  .login-form {
    ...
    background: -webkit-gradient(
      ...
      left top,
      left bottom,
      color-stop(0%, $bg_form_top),
      color-stop(100%, $bg_form_bottom)
    );

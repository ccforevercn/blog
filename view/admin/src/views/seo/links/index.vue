<template>
  <div class="app-container">
      ...
      <el-form :model="where" @submit.native.prevent>
        <el-col :span="16">
          <el-form-item label="网站权重传递状态">
            <el-select
              v-model="where.follow"
              placeholder="请选择网站权重传递状态"
              @change="setWhere"
            >
              <el-option label="全部菜单" value="" />
              <el-option label="禁止" value="0" />
              <el-option label="传递" value="1" />
            </el-select>
          </el-form-item>
        </el-col>
      </el-form>
    ...
    >
      ...
      <el-table-column label="传权">
        <template slot-scope="scope"
          ><el-tag :type="scope.row.follow | followTypeFilter" class="cursor">{{
            scope.row.follow | followFilter
          }}</el-tag></template
        >
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    deleteDialog(index) {
      ...
      that
        .$confirm("您要永久删除【" + links.name + "】友情链接吗?", "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning",
        })
        .then(() => {
          DeleteDelete({ id: links.id }).then(() => {
            that.getList();
          });
        })
        .catch(() => {

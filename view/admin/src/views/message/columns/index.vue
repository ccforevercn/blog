    ...
    >
      ...
      <el-table-column label="信息">
        <template slot-scope="scope">
          ...
          ><br />
          <span>别名:{{ scope.row.name_alias }}</span
          ><br />
          <span>父级栏目:{{ scope.row.parentname }}</span
          ><br />
      ...
      <el-table-column align="center" label="banner图片" width="80">
        <template slot-scope="scope"
          ><span class="cursor-pointer"
            ><el-image
              class="table-image"
              @click="openImage(imageUrl + scope.row.banner_image)"
              :src="imageUrl + scope.row.banner_image"
              fit="scale-down"
            ></el-image></span
        ></template>
      </el-table-column>
      ...
      <el-table-column align="center" label="信息排序" width="120">
        <template slot-scope="scope"
          ><el-tag :type="scope.row.sort | sortTypeFilter" class="cursor">{{
            scope.row.sort | sortFilter
          }}</el-tag></template
        >
      </el-table-column>
      <el-table-column align="center" label="导航状态" width="80">
        <template slot-scope="scope"
          ><el-tag
            :type="scope.row.navigation | navigationTypeFilter"
            class="cursor"
            >{{ scope.row.navigation | navigationFilter }}</el-tag
          ></template
        >
      </el-table-column>
      <el-table-column align="center" label="操作" width="180">
        <template slot-scope="scope">
          <el-button-group>
            ...
            <el-tooltip content="内容修改" placement="top">
              <el-button
                type="success"
    ...
    >
      <el-form class="edit-dialog-form">
        ...
        <el-form-item label="别名">
          <el-input v-model="editInfo.name_alias" />
        </el-form-item>
        <el-form-item label="父级栏目">
          <el-select
        ...
        <el-form-item label="banner图片">
          <upload-image
            ...
            @setImagePath="setBannerImagePath"
          />
        </el-form-item>
        ...
        <el-form-item label="信息每页条数(值为0时，信息栏目列表页不查询)">
          <el-input v-model="editInfo.limit" type="number" />
        </el-form-item>
        <el-form-item label="信息排序">
          <el-select v-model="editInfo.sort" placeholder="信息排序" filterable>
            <el-option label="编号倒叙" value="0" />
            <el-option label="修改时间升序" value="1" />
            <el-option label="修改时间倒叙" value="2" />
            <el-option label="权重升序" value="3" />
            <el-option label="权重倒叙" value="4" />
            <el-option label="点击量升序" value="5" />
            <el-option label="点击量降序" value="6" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="页面">
          <el-input
            v-show="renderType === false"
            v-model="editInfo.page"
            type="url"
          />
          <el-select
            v-show="renderType === true"
            v-model="editInfo.page"
            placeholder="页面"
            filterable
          >
...
export default {
  ...
  data() {
    return {
      ...
      imagePath: "columns",
      bannerImage: "",
      bannerImageName: "columnsbanner",
      bannerImagePath: "columnsbanner",
  ...
  methods: {
    ...
    getContent(id, callback) {
      ...
      that.editContent.content.markdown = "";
      that.editContent.content.content = "";
      console.log(that.editContent.content);
    ...
    deleteDialog(index) {
      ...
      that
        .$confirm("您要永久删除【" + column.name + "】栏目吗?", "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning",
        })
        .then(() => {
          DeleteDelete({ id: column.id }).then(() => {
            that.getColumns();
            that.getList();
          });
        })

 ...
 * @package App
 */
class ConfigCategory implements InterfacesModel
{
    use TraitsModel;
    ...
    public function lst(array $where, array $order, int $offset, int $limit): array
    {
        ...
        try{
            $sql = "SELECT ";
            $sql .= $this->select(['id', 'name', 'description', 'add_time'], []);
            ...
            return array_map(function ($item){
                return ['id' => $item->id, 'name' => $item->name, 'description' => $item->description, 'add_time' => date('Y-m-d H:i', $item->add_time)];
            }, $list);

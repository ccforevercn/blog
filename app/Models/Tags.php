 ...
 * @package App\Models
 */
class Tags implements InterfacesModel
{
    use TraitsModel;
    ...
    public function lst(array $where, array $order, int $offset, int $limit): array
    {
        ...
        try{
            // 查询字段
            $selects = ['id', 'name', 'add_time', 'status'];
            $sql = "SELECT ";
            ...
            return array_map(function ($item){
                return ['id' => $item->id, 'name' => $item->name, 'add_time' => date('Y-m-d H:i', $item->add_time), 'status' => $item->status];
            }, $list);
    ...
    {
        try {
            $sql = "SELECT COUNT(id) as count FROM ".$this->prefix.$this->table." WHERE id IN(".implode(',', $ids).") AND is_del = 0";

 ...
 * @package App
 */
class Links implements InterfacesModel
{
    use TraitsModel;
    ...
    public function lst(array $where, array $order, int $offset, int $limit): array
    {
        ...
        try{
            // 查询字段
            $selects = ['id', 'name', 'link', 'image', 'weight', 'follow', 'add_time'];
            $sql = "SELECT ";
            ...
            return array_map(function ($item){
                return ['id' => $item->id, 'name' => $item->name, 'link' => $item->link,'image' => $item->image, 'weight' => $item->weight, 'follow' => $item->follow, 'add_time' => date('Y-m-d H:i', $item->add_time)];
            }, $list);
